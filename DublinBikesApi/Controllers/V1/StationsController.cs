...
namespace DublinBikesApi.Controllers.V1;

/// <summary>
/// V1 API Controller for Dublin Bikes Stations (JSON file based)
/// </summary>
[ApiController]
...
public class StationsController : ControllerBase
{
    ...
    {
        var cacheKey = $"stations_v1_{status}_{minBikes}_{q}_{sort}_{dir}_{page}_{pageSize}";

        var cachedResult = _cacheService.Get<PagedResponse<StationDto>>(cacheKey);
        if (cachedResult != null)
        {
            _logger.LogDebug("Returning cached result for GetStations");
    ...
    {
        var cacheKey = $"station_v1_{number}";

        var cachedResult = _cacheService.Get<StationDto>(cacheKey);
        if (cachedResult != null)
        {
            _logger.LogDebug($"Returning cached result for station {number}");
            return Ok(cachedResult);
        }
        ...
        if (station == null)
        {
            _logger.LogWarning($"Station {number} not found");
    ...
    {
        var cacheKey = "stations_summary_v1";

        var cachedResult = _cacheService.Get<StationsSummaryDto>(cacheKey);
        if (cachedResult != null)
        {
            _logger.LogDebug("Returning cached result for GetSummary");
            return Ok(cachedResult);
        }
    ...
    public async Task<ActionResult<StationDto>> CreateStation([FromBody] CreateStationDto createDto)
    {
        ...
        {
            _logger.LogWarning(ex, "Failed to create station");
        ...
        [FromBody] UpdateStationDto updateDto)
    {
        ...
        if (station == null)
        {
            _logger.LogWarning($"Station {number} not found for update");
            return NotFound(new { message = $"Station with number {number} not found" });
        }
